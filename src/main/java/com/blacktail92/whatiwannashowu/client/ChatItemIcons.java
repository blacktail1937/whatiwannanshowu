 ...
 * {@link FormattedCharSequence}，撞到"两个连续空格"就把它们前面的文字宽度算出来，
 * 在那个位置用 {@link GuiGraphics#renderItem} 画<b>真正的物品模型</b>。
 *
 * <p>这样做的最大好处：走的是原版物品渲染管线，所以附魔光效、染色、多图层、3D 模型
 * 全都是对的 —— 而不是一张扁平贴图。
 */
public final class ChatItemIcons {
     ...
     *
     * <p>左侧间隙写进占位里，而不是靠消息前缀 {@code "<名字> "} 里那个空格：整行第一段连续空格
     * 就是从占位这里开始的，图标画在空格段起点右移 {@link #ICON_OFFSET_SPACES} 个空格处，
     * 于是左右两侧各留 4px。这样写死以后，前缀带不带空格都不会把右侧间隙挤没。
     */
     ...
     *
     * <p>这里<b>不判断</b> {@code renderItemsInChat} 开关：那个开关只决定"新分享的消息带不带图标占位"，
     * 已经在聊天栏里的消息（占位还在文字里）应该继续显示图标，否则一关开关老消息的图标就全没了。
     */
    ...
    private static void render(Minecraft mc, GuiGraphics guiGraphics, String before,
        ...
        pose.pushPose();
        // 只做"平移到占位处 + 缩放到 8px"：当前的 pose 就是这一行文字用的那套变换，
        // 所以不能再往里套一层 —— 早期照抄 Quark 的 mulPose(last().pose()) 会把聊天栏的变换
        // 平方掉，在整合包里（聊天栏被别的 mod 用更复杂的变换渲染时）图标会被推到可见范围外，
        // 表现成"代码执行了、屏幕上看不见"。
